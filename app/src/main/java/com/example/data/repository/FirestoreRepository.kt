...
class FirestoreRepositoryImpl(
    ...
    override suspend fun deleteUserData(uid: String): Result<Unit> = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        try {
            ...
            // 1. Delete users/{uid}
            try { firestore.collection("users").document(uid).delete().await() } catch (e: Exception) { if (e is kotlinx.coroutines.CancellationException) throw e; Log.w("FirestoreRepo", "Error deleting users/$uid: ${e.message}") }

            // 2. Delete wallet/{uid}
            try { firestore.collection("wallet").document(uid).delete().await() } catch (e: Exception) { if (e is kotlinx.coroutines.CancellationException) throw e; Log.w("FirestoreRepo", "Error deleting wallet/$uid: ${e.message}") }

            // 3. Delete rewards/{uid}
            try { firestore.collection("rewards").document(uid).delete().await() } catch (e: Exception) { if (e is kotlinx.coroutines.CancellationException) throw e; Log.w("FirestoreRepo", "Error deleting rewards/$uid: ${e.message}") }

            // 4. Delete transactions/{uid} and user transactions
            try { firestore.collection("transactions").document(uid).delete().await() } catch (e: Exception) { if (e is kotlinx.coroutines.CancellationException) throw e; Log.w("FirestoreRepo", "Error deleting transactions/$uid: ${e.message}") }
            try {
                val transQuery = firestore.collection("transactions").whereEqualTo("userId", uid).get().await()
                for (doc in transQuery.documents) {
                    doc.reference.delete().await()
                }
            } catch (e: Exception) { if (e is kotlinx.coroutines.CancellationException) throw e; Log.w("FirestoreRepo", "Error deleting user transactions query: ${e.message}") }

            // 5. Delete user_daily_state/{uid}
            try { firestore.collection("user_daily_state").document(uid).delete().await() } catch (e: Exception) { if (e is kotlinx.coroutines.CancellationException) throw e; Log.w("FirestoreRepo", "Error deleting user_daily_state/$uid: ${e.message}") }
            ...
            } catch (e: Exception) { if (e is kotlinx.coroutines.CancellationException) throw e; Log.w("FirestoreRepo", "Error deleting device_bindings: ${e.message}") }
