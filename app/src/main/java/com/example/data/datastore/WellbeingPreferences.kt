...
class WellbeingPreferences(private val context: Context) {
    ...
    companion object {
        ...
        val LAST_SESSION_LOGIN_TIME = longPreferencesKey("last_session_login_time")
        val ACTIVE_DEVICE_ID = stringPreferencesKey("active_device_id")
    }
    ...
    val lastSessionLoginTimeFlow: Flow<Long> = context.dataStore.data.map { preferences ->
        preferences[LAST_SESSION_LOGIN_TIME] ?: 0L
    }

    val activeDeviceIdFlow: Flow<String> = context.dataStore.data.map { preferences ->
        preferences[ACTIVE_DEVICE_ID] ?: ""
    }
    ...
    suspend fun setSessionInfo(deviceId: String, loginTime: Long) {
        context.dataStore.edit { preferences ->
            preferences[ACTIVE_DEVICE_ID] = deviceId
            preferences[LAST_SESSION_LOGIN_TIME] = loginTime
        }
    }
