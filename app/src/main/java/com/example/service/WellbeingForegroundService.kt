            ...
            while (isActive) {
                ...
                try {
                    ...
                    if (uid != null) {
                        ...
                        val preferences = app?.container?.preferences
                        val lastKnownLoginTime = preferences?.lastSessionLoginTimeFlow?.firstOrNull() ?: 0L
                        ...
                        if (userResult.isSuccess) {
                            ...
                            if (profile != null && profile.sessionActive && profile.activeDeviceId.isNotEmpty()) {
                                if (profile.activeDeviceId == currentDeviceId) {
                                    if (profile.lastLoginTime > lastKnownLoginTime) {
                                        preferences?.setSessionInfo(currentDeviceId, profile.lastLoginTime)
                                    }
                                } else {
                                    // Device mismatch detected. Only trigger force logout if remote profile login timestamp is newer than our local login timestamp
                                    val isRemoteSessionNewer = lastKnownLoginTime > 0L && profile.lastLoginTime > (lastKnownLoginTime + 2000L)
                                    if (isRemoteSessionNewer) {
                                        Log.w("WellbeingFGS", "Remote session takeover confirmed! Active device id: ${profile.activeDeviceId}, current: $currentDeviceId. FORCING LOGOUT.")
                                        ...
                                        Log.d("WellbeingFGS", "Device ID differs (${profile.activeDeviceId} vs $currentDeviceId), but local session is active/pending sync.")
                                    }
                                }
                            }
                        }
